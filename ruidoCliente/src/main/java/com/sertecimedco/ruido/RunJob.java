...
public class RunJob implements Job {
	...
	public void execute(JobExecutionContext arg0) throws JobExecutionException {
		logger.info("Iniciando el demonio...");
		...
		if (NetUtil.thereIsPing(Run.SERVER_IP) && comunArchivos.obtieneListadoArchivos()) {
			...
			for (String nombreFicheroDate : comunArchivos.listadoFicherosDate) {
				...
				if(ficheroFecha.exists() && ficheroFecha.isDirectory()) {
					...
					logger.debug("Procesando fichero date: " + nombreFicheroDate);
	...
	private void hayArchivoLLamada(String nombreArchivo, String nombreFecha) {
		logger.info("Ejecutando llamada a hay archivo");
	...
	private void subeArchivoLLamada(String nombreArchivo, String nombreFecha) {
		logger.info("Ejecutando llamada a sube archivo");
		...
		try {
			...
			String pathArchivo = Run.pathPorProcesar + File.separator
					+ nombreFecha + File.separator + nombreArchivo;
			
			sar.setEstacion(Run.ESTACION);
			sar.setMd5(MD5.fileMd5Sum(new File(pathArchivo)));
			sar.setNombreArchivo(nombreArchivo);
			sar.setNombreFecha(nombreFecha);
	...
	private void moverArchivo(String nombreArchivo, String nombreFecha) {
